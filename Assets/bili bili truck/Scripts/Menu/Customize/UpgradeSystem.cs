...
namespace BilgeKorkmaz
{
	...
	public class UpgradeSystem : MonoBehaviour
	{
		...
		public void LoadUpgrade()
		{
			...
			tBrake = PlayerPrefs.GetInt("Brake" + PlayerPrefs.GetInt("TruckID"));
		...
		public void BrakeUpgrade()
		{
			if (tBrake < maxUpgradeLevel)
			{
				if (PlayerPrefs.GetInt("Coins") > brakePrice[tBrake])
				{
					...
					tBrake++;
					PlayerPrefs.SetInt("Brake" + PlayerPrefs.GetInt("TruckID").ToString(), tBrake);
					tMoney.UpdateDisplay();
		...
		{
			if (PlayerPrefs.GetInt("Truck" + GameObject.FindFirstObjectByType<VehicleSelect>().ID.ToString()) == 3)
			{
