...
namespace BilgeKorkmaz
{
	...
	public class T_RingSelection : MonoBehaviour
	{
		...
		{
			if (PlayerPrefs.GetInt(rings.carID.ToString() + "Ring" + id.ToString()) == 3)
			{  // 3=>true , 0=>false

				// Find current car T_Rings component
				rings = GameObject.FindFirstObjectByType<T_Rings>();
			...
			{// The current selected ring is not unlocked, We unlock it with reducing money from player total moneys and unlock ring
